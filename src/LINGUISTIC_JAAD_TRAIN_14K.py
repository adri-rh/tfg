...
class JAAD(InMemoryDataset):
    ...
    def process(self):
        def create_graphs(csv_path, desc):
            ...
            n_rows = len(df.index)
            n_feats = 7 #6 para numeric, 7 para linguistic
        ...
        data_list = create_graphs('datasets/LINGUISTIC_JAAD_TRAIN_14K.csv', "Procesando JAAD (train)")
        self.save(data_list, self.processed_paths[0])
        data_list_test = create_graphs('datasets/LINGUISTIC_TEST_JAAD_ALL.csv', "Procesando JAAD (test)")
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(
        project="tfg",
        name="GraphConv_LINGUISTIC_JAAD_TRAIN_14K_SlidingWindows_Combinación",
        log_model=True
    )
